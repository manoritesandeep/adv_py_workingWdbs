from sqlalchemy import Column, String, Integer, Numeric, create_engine, ForeignKey, select, func
...
engine = create_engine(f'mysql+mysqlconnector://{user}:{password}@localhost:3306/{db_name}',
                                  echo=True)
...
class Sale(Base):
    ...
    def __repr__(self):
        return "<Sale(order_num='{0}', cust_name='{1}', prod_name='{2}', quantity='{3}')".format(self.order_num, self.cust_name, self.prod_name, self.quantity, self.order_total)
